...
import { removeFromFav,isFav, addToFav } from './fav';
...
function MovieCard({ movie }) {
  ...
  return (
    <div className='bg-white rounded-lg shadow hover:shadow-xl transition overflow-hidden relative'>
      <button onClick={toggleFav} className='absolute top-2 right-2 text-2xl'>{fav? "❤️" : "🤍"}</button>
      <img 
      ...
      <div className='p-4'>
        ...
        <div className='flex justify-between items-center mt-2'>
          <span className='bg-yellow-400 px-2 py-1 rounded text-sm font-bold'>⭐ {movie.vote_average}</span>
